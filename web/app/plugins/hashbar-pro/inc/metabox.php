...
if( !function_exists('hashbar_custom_date_field_callback') ){
	function hashbar_custom_date_field_callback( $args ){
		...
		?>
		<input type="text" name="_wphash_[<?php echo $field; ?>]" value="<?php echo esc_attr($field_value); ?>" data-depend-id="<?php echo esc_attr($field); ?>" id="<?php echo esc_attr($post_id).esc_attr($field).'-pro'; ?>">
		<div class="clear"></div>
		...
		<script>
			;( function ( $ ) {
			    ...
			    $(document).ready(function(){
	    		    $( "#<?php echo $post_id.$field.'-pro'; ?>" ).datetimepicker({
	    		    	controlType: 'select',
	    	    		oneLine: true,
	    	    		dateFormat: 'mm/dd/yy',
	    	    		timeFormat: 'hh:mm tt',
...
if( class_exists( 'CSF' ) ) {
	...
	// Limit
	$hashbar_options = get_option( 'hashbar_wpnbp_opt');
	...
	if(!hashbar_wpnbp_is_classic_editor_plugin_active()){
	...
	// Select Categories
	$section_1_fields[] = array(
		'id'          => $prefix. 'notification_where_to_show_Categories',
		'type'        => 'select',
		'title'       => esc_html__( 'Choose categories', 'hashbar' ),
		'desc'        => esc_html__('This notification will be displayed on all blog detail pages in the selected categories mentioned above.', 'hashbar'),
		'placeholder' => esc_html__('Select Categories'),
		'options'     => 'categories',
		'multiple'    => true,
		'chosen'      => true,
		'ajax'        => $enable_ajax_select,
		'dependency'  => array( $prefix. 'notification_where_to_show', '==', 'post_cat' ),
	);

	// Select Tags
	$section_1_fields[] = array(
		'id'          => $prefix. 'notification_where_to_show_Tags',
		'type'        => 'select',
		'title'       => esc_html__( 'Choose tags', 'hashbar' ),
		'desc'        => esc_html__('This notification will be displayed on all blog detail pages in the selected tags mentioned above.', 'hashbar'),
		'placeholder' => esc_html__('Select Tags'),
		'options'     => 'tags',
		'multiple'    => true,
		'chosen'      => true,
		'ajax'        => $enable_ajax_select,
		'dependency'  => array( $prefix. 'notification_where_to_show', '==', 'post_tags' ),
	);
	...
	$section_1_fields[] = array(
		...
		'desc'    => esc_html__( 'Control visibility of the close icon/button.', 'hashbar' ),
		'options' => array(
	...
	// 	'type'       => 'text',
	// 	'title'      => esc_html__( 'Notification bar height (px)', 'hashbar' ),
	// 	'desc'       => __( 'Input Height of notification bar, Example: 300. It must need if your theme\'s header type is transparent/sticky', 'hashbar' ),
	// 	'dependency' => array( '_wphash_notification_position|_wphash_themes_header_type', '==|==', 'ht-n-top|transparent'),
	// );

	// $section_1_fields[] = array(
	// 	'id'         =>  '_wphash_notification_mobile_height',
	// 	'type'       => 'text',
	// 	'title'      => esc_html__( 'Notification bar for mobile height (px)', 'hashbar' ),
	// 	'desc'       => __( 'Input Mobile Height of notification bar, Example: 300. It must need if your theme\'s header type is transparent/sticky', 'hashbar' ),
	// 	'dependency' => array( '_wphash_notification_position|_wphash_themes_header_type', '==|==', 'ht-n-top|transparent'),
	// );
	...
	$visibility_fields[] = array(
		...
		'dependency'		 => array( $prefix. 'notification_schedule', '==', 'on' ),
	);
	CSF::createSection( $prefix, array(
		'id'     => 'genearl_settings',
	...
	$design_fields[] = array(
		...
		'type'  => 'color',
		'title' => esc_html__( 'Hover Background color', 'hashbar' ),
	);
