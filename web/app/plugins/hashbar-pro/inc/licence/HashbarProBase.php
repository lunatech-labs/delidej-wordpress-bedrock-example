<?php
if(!class_exists("Hashbar_Pro_Base")) {
	class Hashbar_Pro_Base {
    	public $key = "AC86A4EF80C63335";
    	private $product_id = "12";
    	private $product_base = "hashbar-pro";
    	//private $server_host = "https://license.hasthemes.com/wp-json/licensor/";
		private $server_host = "https://license.htcode.biz/api/";
		...
		public static $woolentorLicenseData;
		// Static property to cache API response for the entire request lifecycle
		private static $htcode_biz_api_response_cache = null;
		function __construct($plugin_base_file='')
		{
			if(empty($plugin_base_file)){
				$dir=str_replace('\\','/',dirname(HASHBAR_WPNBP_ROOT));
			}else{
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_hashbar-pro_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=hashbar-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		public static function setWooLentorData(){
			// Check If WooLentor license is active or not
			if( class_exists('WooLentorPro\Base') ){
	            if( !class_exists('WooLentorProBase') ){
	                require_once( WOOLENTOR_ADDONS_PL_PATH_PRO.'includes/licence/WooLentorProBase.php' );
	            }
	            $licenseKey = get_option("WooLentorPro_lic_Key","");
	            $liceEmail 	= get_option( "WooLentorPro_lic_email","");
	            $licenseMessage = null;
	            WooLentorProBase::CheckWPPlugin( $licenseKey, $liceEmail, $licenseMessage, self::$woolentorLicenseData, WOOLENTOR_ADDONS_PL_ROOT_PRO );
	        }
		}
		public function isWooLentorBundle(){
			$dataObj = self::$woolentorLicenseData;
			if( $dataObj ){
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","hashbar-pro").'</h1>');
		...
		function get_plugin_update_data(){
			// Check if we already have a response cached for this request lifecycle
			if (self::$htcode_biz_api_response_cache !== null) {
				return self::$htcode_biz_api_response_cache;
			}
			
			$dataObj = self::$woolentorLicenseData;
			$url = $this->server_host . 'v1/getproduct';
		
			$request_args = array(
				'method'  => "GET",
				'body'    => array( 'is_valid'=> $dataObj->is_valid, 'license_key'=> $dataObj->license_key,'license_title'=> $dataObj->license_title, 'product_id'=>$this->product_id, 'server_host' => $this->server_host)
			);
			...
			if ( is_object($response) ) {
				...
				$data = (object)[
					...
					'new_version'=> $responseJson->new_version,
					'url'=>"",
					'download_link'=>$responseJson->download_link,
					'package' => $responseJson->download_link,
					'icons'         => array(),
					'banners'       => array(),
					'banners_rtl'   => array(),
					'tested'        => $responseJson->tested,
					'requires'      => $responseJson->required_version,
					'sections'      => array(
						'description' =>$responseJson->update_des
					),
					'last_updated' =>$responseJson->update_time
				];
				
				// Store in static cache for this request lifecycle
				self::$htcode_biz_api_response_cache = $data;
		
				return $data;
			}
			
			// Cache the null response too to avoid multiple API calls in the same request
			self::$htcode_biz_api_response_cache = null;
		}
		function el_plugin_update_info(){
            if(function_exists("wp_remote_get")) {
				if($this->isWooLentorBundle()){
					return $this->get_plugin_update_data();
				}
                ...
                if (is_wp_error($server_response)) {
                    ...
                    $server_response = wp_remote_post($url, $rq_params);
                     if (is_wp_error($server_response)) {
						...
						if ( !empty( $server_response['body'] ) && $server_response['body'] != "GET404" ) {
                ...
                if ( empty( $response->code ) ) {
                    if ( ! empty( $response->status ) ) {
                        if ( ! empty( $response->data ) ) {
                            ...
                            //$license_obj = unserialize( $serial_obj ); //Old code for api server
							$license_obj = json_decode( $serial_obj ); // for our api
                            if ( $license_obj->is_valid ) {
                                $response_obj           = new stdClass();
                                $response_obj->is_valid = $license_obj->is_valid;
								$request_duration = max(168, absint($license_obj->request_duration));
								$response_obj->next_request = strtotime("+ {$request_duration} hour");
        ...
        private function _check_old_tied(&$old_respons,&$response_obj){
            if(!empty($old_respons) && (empty($old_respons->tried) || $old_respons->tried<=2)){
                $old_respons->next_request = strtotime("+ 18 hour");
