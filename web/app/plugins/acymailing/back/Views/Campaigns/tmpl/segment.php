		...
		<div class="cell <?php echo $data['containerClass']; ?> float-center grid-x acym__content">
            ...
            if (acym_level(ACYM_ENTERPRISE)) {
				...
				<div class="cell grid-x acym__campaigns__segment__summary margin-bottom-1">
					<h4 class="cell acym__title acym__title__secondary">
                        <?php echo acym_translation('ACYM_CHOSEN_LISTS').acym_info(['textShownInTooltip' => 'ACYM_CHOSEN_LISTS_DESC']); ?>
					</h4>
							...
							<div class="cell grid-x grid-margin-x">
								...
								<div class="cell shrink">
                                    ...
                                    foreach ($data['campaignLists'] as $list) {
                                        echo acym_tooltip(
                                            [
                                                'hoveredText' => '<i class="acymicon-circle" style="color: '.$list->color.'"></i>',
				...
				<div class="cell grid-x acym__campaigns__segment__edit margin-top-1">
					<h4 class="cell acym__title acym__title__secondary">
                        <?php echo acym_translation('ACYM_SEGMENT').acym_info(['textShownInTooltip' => 'ACYM_SEGMENT_CAMPAIGN_DESC']); ?>
					</h4>
					<div class="cell grid-x padding-1">
						<div class="cell grid-x acym_vcenter">
							<p class="cell shrink margin-next-1"><?php echo acym_translation('ACYM_SELECT_EXISTING_SEGMENT'); ?></p>
						...
						<div class="cell grid-x acym_vcenter padding-top-1">
                            <?php
                            echo acym_radio(
                                    ['include' => acym_translation('ACYM_INCLUDE'), 'exclude' => acym_translation('ACYM_EXCLUDE')],
                                    'invert',
                                    empty($data['campaign']->sending_params['segment']['invert']) ? 'include' : $data['campaign']->sending_params['segment']['invert'],
                                    ['class' => 'acym__radio__exclude']
                                ).acym_info(['textShownInTooltip' => 'ACYM_SEGMENT_INCLUDE_EXCLUDE']);
                            ?>
						</div>
						...
						<div class="cell grid-x">
							...
							<div class="cell grid-x acym__content margin-top-1 margin-bottom-1 acym__campaigns__segment__edit__filters">
                                <?php if (acym_isAllowed('segments')) { ?>
									<div class="acym__campaigns__segment__edit__filters__save">
										<div class="acym__campaigns__segment__edit__filters__save-icon acym_vcenter grid-x align-right">
                                            ...
                                            echo acym_tooltip(
                                                [
                                                    'hoveredText' => '<i class="acym__color__blue acymicon-floppy-o cell shrink"></i>',
                                                    'textShownInTooltip' => acym_translation('ACYM_SAVE_SEGMENT'),
                                                ]
                                            );
                                            ?>
										</div>
										<div class="acym__campaigns__segment__edit__filters__save-action grid-x acym_vcenter align-right">
											<label for="acym__campaigns__segment__edit__filters__save-segment-name" class="cell shrink">
                                                <?php echo acym_translation('ACYM_SEGMENT_NAME'); ?>
												:</label>
											<input type="text"
												   class="cell shrink"
												   id="acym__campaigns__segment__edit__filters__save-segment-name"
												   name="segment_name">
											<button type="button" class="button cell shrink acym__campaigns__segment__edit__filters__save-action-save">
								...
								<input type="hidden"
									   name="list_selected"
									   value="<?php echo empty($data['campaignLists']) ? '' : json_encode(array_keys($data['campaignLists'])); ?>">
								<input type="hidden"
							...
							<div class="cell grid-x acym__content acym_vcenter acym__campaigns__segment__edit__count_container">
								...
								<p class="medium-6 cell acym_vcenter align-right"><span id="acym__campaigns__segment__edit-user-count"></span>
                                    <?php echo acym_strtolower(acym_translation('ACYM_SUBSCRIBERS')); ?></p>
							</div>
						</div>
					</div>
				</div>
